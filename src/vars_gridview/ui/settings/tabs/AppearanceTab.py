...
class AppearanceTab(AbstractSettingsTab):
    ...
    def __init__(self, settings: AppSettings | None = None, parent=None):
        ...
        self._set_theme_combo_value(self._settings.gui_style.value)
        self.theme_combo_box.currentIndexChanged.connect(self.settingsChanged.emit)
        self._settings.gui_style.valueChanged.connect(self._set_theme_combo_value)
    ...
    def apply_settings(self) -> set[str]:
        ...
        if self._settings.label_font_size.set_value(
            self.label_font_size_spinbox.value()
        ):
            changed.add(self._settings.label_font_size.key)
        if self._settings.selection_highlight_color.set_value(
